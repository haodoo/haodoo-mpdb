...
uses Dialogs, SysUtils, Classes, Types, zlib, Windows;
...
//        ShowMessage(filename + ':'+ IntToStr(CommonFileHeader.FilenameLength)); // JJ
...
begin
  ...
  {$ENDIF}
  try {+}
    if Compress then CL := clDefault else CL := clNone;

    compressor := TcompressionStream.Create(CL, CompressedStream);
    try {+}
...
//    ShowMessage(IntToStr(compressedStream.Size));
    if Compress then head := 2 else head := 7; // JJ
    for ii := head to compressedStream.Size - 5 do
  ...
  {$ENDIF}
    ...
    begin
      ...
      if Compress then CompressionMethod := 8 else CompressionMethod := 0;
